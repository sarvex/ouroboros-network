                     ...
                     (\connHandle -> do
                      case connHandle of
                        Connected connId _ (Handle mux muxBundle controlBundle _
                                        :: HandleP2P InitiatorMode peerAddr
...
multinodeExperiment inboundTrTracer trTracer inboundTracer cmTracer
  ...
  where
    ...
    connectionLoop
         ...
         -- ^ control channel
         -> ConnectionManagerP2P muxMode socket peerAddr DataFlowProtocolData UnversionedProtocol ByteString m [resp] a
         -> Map.Map peerAddr (HandleP2P muxMode peerAddr DataFlowProtocolData ByteString m [resp] a)
         -- ^ active connections
    ...
    connectionLoop muxMode localAddr cc cm connMap0 connVar = go True connMap0
      where
        go :: Bool -- if false do not run 'unregisterOutboundConnection'
           -> Map.Map peerAddr (HandleP2P muxMode peerAddr DataFlowProtocolData ByteString m [resp] a) -- active connections
           -> m ()
